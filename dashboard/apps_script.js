function doPost(e) {
  try {
    ...
    // ── setStrategyChange: writes ONLY col L (Strategy Changed On) in Clients Master ──
    if (action === 'setStrategyChange') {
      const amName     = data.am_name     ? String(data.am_name).trim()     : null;
      const changeDate = data.change_date ? String(data.change_date).trim() : null;
      if (!amName || !changeDate) {
        return ContentService
          .createTextOutput(JSON.stringify({success: false, error: 'am_name and change_date required'}))
          .setMimeType(ContentService.MimeType.JSON);
      }
      const AM_SHEET_ID = '12KEc1_CIkAHpfA74y660zsWSGnkbcSoltcSosuk4smA';
      const masterSS    = SpreadsheetApp.openById(AM_SHEET_ID);
      const masterSheet = masterSS.getSheetByName('Clients Master');
      if (!masterSheet) {
        return ContentService
          .createTextOutput(JSON.stringify({success: false, error: 'Clients Master tab not found'}))
          .setMimeType(ContentService.MimeType.JSON);
      }
      const lastMasterRow = masterSheet.getLastRow();
      if (lastMasterRow < 3) {
        return ContentService
          .createTextOutput(JSON.stringify({success: false, error: 'No client rows in master'}))
          .setMimeType(ContentService.MimeType.JSON);
      }
      const colA     = masterSheet.getRange(3, 1, lastMasterRow - 2, 1).getValues();
      const normName = amName.toLowerCase().replace(/\s+/g, ' ');
      let targetRow  = -1;
      for (let i = 0; i < colA.length; i++) {
        if (String(colA[i][0]).trim().toLowerCase().replace(/\s+/g, ' ') === normName) {
          targetRow = i + 3;
          break;
        }
      }
      if (targetRow === -1) {
        return ContentService
          .createTextOutput(JSON.stringify({success: false, error: 'am_name not found: ' + amName}))
          .setMimeType(ContentService.MimeType.JSON);
      }
      masterSheet.getRange(targetRow, 12).setValue(changeDate);  // col L only
      // Append note to col K (Notes) if provided
      if (data.note && String(data.note).trim()) {
        var noteText  = String(data.note).trim();
        var dateStamp = Utilities.formatDate(new Date(), 'Asia/Amman', 'dd/MM/yyyy');
        var existing  = String(masterSheet.getRange(targetRow, 11).getValue()).trim();
        masterSheet.getRange(targetRow, 11).setValue(
          existing ? existing + ' | [' + dateStamp + '] ' + noteText : '[' + dateStamp + '] ' + noteText
        );
        Logger.log('Appended note to col K for ' + amName);
      }
      Logger.log('setStrategyChange: ' + amName + ' row=' + targetRow + ' date=' + changeDate);
      return ContentService
        .createTextOutput(JSON.stringify({success: true, am_name: amName, change_date: changeDate, row: targetRow}))
        .setMimeType(ContentService.MimeType.JSON);
    }
